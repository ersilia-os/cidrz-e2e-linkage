...
class Vectoriser:
    ...
    def __init__(self, vectoriser=None):
        ...
        if vectoriser is None:
            self._vectoriser = TfidfVectorizer(analyzer="char_wb", ngram_range=(2, 2), max_features=200)
        else:
...
def get_matching_blocks(query, query_field, target, target_field):
    ...
    block.add_with_ids(target_matrix.toarray().astype('float32'), np.array(target.index))
...
def test():
    ...
    ca_df = pd.read_csv('/Users/mwansa.lumpa/PhD/Datasets/SmartCare/toshare/final_to_share/ca_original_clean.csv')
    sc_df = pd.read_csv('/Users/mwansa.lumpa/PhD/Datasets/SmartCare/toshare/final_to_share/sc_original_clean.csv')

    ca_to_match = pd.DataFrame()
    sc_to_match = pd.DataFrame()

    ca_to_match['vector_field'] = ca_df.loc[(ca_df['c_name'].notna()) &
                                      (ca_df['birth_year_coalesce'].notna())]['c_name'] \
                                      + ' ' + ca_df.loc[(ca_df['c_name'].notna()) &
                                                  (ca_df['birth_year_coalesce'].notna())]['birth_year_coalesce'].astype('int32').astype(str)

    sc_to_match['vector_field'] = sc_df.loc[(sc_df['full_name'].notna()) & (sc_df['birth_year'].notna())]['full_name']\
                                  + ' ' + \
                                  sc_df.loc[(sc_df['full_name'].notna()) & (sc_df['birth_year'].notna())]['birth_year'].astype('int32').astype(str)



    blocks = get_matching_blocks(ca_to_match, 'vector_field', sc_to_match, 'vector_field')
    ...
    return ca_to_match.index, ca_df, blocks, sc_df#TODO for the ca_df, filter only to those that had matches
