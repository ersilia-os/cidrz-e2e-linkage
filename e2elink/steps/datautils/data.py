...
class Data:
    ...
    def column_summary(self, column_name):
        ...
        return (
            column_name,
            self.data[column_name].count(),
            self.data[column_name].nunique(),
        )
    ...
    def compare_to_counts(self, secondobj, comp_mapping):
        ...
        for orig_col_name, mapped_col_name in comp_mapping.items():
            results = self.comp_table_inserts(
                results,
                orig_col_name,
                "cleaned",
                "number unique",
                secondobj[mapped_col_name].nunique(),
            )
            results = self.comp_table_inserts(
                results,
                orig_col_name,
                "raw",
                "number unique",
                self.data[orig_col_name].nunique(),
            )
            ...
            results = self.comp_table_inserts(
                ...
                "cleaned",
                "total_counts",
                secondobj[mapped_col_name].count(),
            )
            results = self.comp_table_inserts(
                results,
                orig_col_name,
    ...
    def compare_summaries(self, dataframe, columns=None, column_names=None):
        ...
        print(
            "{:<20}{:<30}{:<30}".format(
                "colname", "before (count, unique)", "after (count, unique"
            )
        )
