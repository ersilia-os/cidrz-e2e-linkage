...
def merge_sources(sourcedir, filemap):
    ...
    for ind, row in mymap.iterrows():
        filename, cols_str, col_names, comments, sheetname, facilityname, visittype = mymap.iloc[ind]

        pathofinterest = file_ca_source + '/' + filename

        dataframe = file_to_dataframe(pathofinterest, cols_str=cols_str, sheetname=sheetname)
