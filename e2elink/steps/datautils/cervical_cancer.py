...
def clean_cx_dob(dataframe, dobfield):
    ...
    for ind, row in dataframe.loc[
        (dataframe[clean_dob_field].isna()) & (dataframe[dobfield].notna()),
        [dobfield, clean_dob_field],
    ].iterrows():
        dataframe.at[ind, clean_dob_field] = _convert_cx_str_to_dob_date(row[dobfield])

    dataframe[clean_dob_field] = pd.to_datetime(
        dataframe[clean_dob_field], errors="coerce"
    )
...
def _correct_cx_dob_year(str_year):
    if len(str_year) == 4:
        ...
        if 20 < int(lastpart) <= 90:  # exludes all years that don't make sense
            year_part = "{0}{1}".format(
                19, lastpart
            )  # assume the last part is always correct if it less than 90
        elif int(lastpart) > 20:
            year_part = "{0}{1}{2}".format(
                19, str_year[3], 5
            )  # if the year is 0195, i convert to 1955
        else:
            return None
    else:
        return None
