...
def clean_via_results(dataframe, colname):
    ...
    dataframe = partial_match_and_replace(
        dataframe, colname, tofindandreplace, default_value="unknown"
    )
...
def partial_match_and_replace(dataframe, colname, kwargs, default_value=np.nan):
    ...
    for key in kwargs:
        dataframe.loc[
            dataframe[colname].apply(lambda x: str(x).lower()).str.contains(key.lower())
            == True,
            tmpcolname,
        ] = kwargs[key]
...
def clean_cacx_reg_full_name(df, colname):
    ...
    df = retrieve_trailing_names_from_date_field(
        df, dt_col="enrol_date", name_col=colname, clean_name_col=clean_name
    )
...
def clean_date_of_screening(df, colname):
    ...
    for ind, row in df.loc[
        (df[clean_dt_col].isna() & df[colname].isna()), [colname, clean_dt_col]
    ].iterrows():
        # print(row[colname])
