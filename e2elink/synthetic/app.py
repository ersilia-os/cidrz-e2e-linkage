...
from e2elink.synthetic.tablegen.linkedtable import (
    ReferenceLinkedTables,
    find_ground_truth,
)
...
src_size = st.sidebar.number_input(
    "Number of samples", min_value=10, max_value=100000, value=100
)
src_dupl = st.sidebar.slider("Proportion of duplicates (%)", 0, 100, 10) / 100
src_visits = st.sidebar.slider(
    "Average visits", min_value=1.0, max_value=5.0, value=1.5, step=0.1
)
...
if src_date_format == "2020-12-31":
    ...
    pass
src_date_format = _df
src_date_missformat_rate = c12.slider("Date missformat (%)", 0, 100, 10)
...
src_name_from = c22.radio("Name origin", ["Local", "Global"], index=0)
...
if src_name_format == "John Smith":
    ...
    pass
src_name_format = _nf
src_name_missformat_rate = c32.slider("Name missformat (%)", 0, 100, 10) / 100
src_name_abbreviation_rate = c31.slider("Abbreviation (%)", 0, 100, 5) / 100
src_name_swapping_rate = c32.slider("Swapping (%)", 0, 100, 5) / 100
src_name_misspelling_rate = c31.slider("Misspelling (%)", 0, 100, 10) / 100
src_name_misspelling_type = c32.radio("Misspelling type", ["Fast", "Accurate"], index=0)
...
if src_age_format == "Date of birth":
    ...
    pass
src_age_format = _af
src_age_missformat_rate = c42.slider("Age missformat (%)", 0, 100, 10) / 100
src_age_range = st.sidebar.slider("Age range (95% CI)", 0, 120, (15, 75))
src_age_lb = src_age_range[0]
src_age_ub = src_age_range[1]
src_age_coverage = st.sidebar.slider("Age coverage (%)", 0, 100, 95) / 100

# Sex
st.sidebar.subheader("Sex")
src_sex_as_binary = st.sidebar.checkbox("Binary variable", value=False)
# Random choices
...
# Identifier
src_do_identifier = st.sidebar.checkbox("Identifier", value=True)
if src_do_identifier:
    src_identifier_coverage = (
        st.sidebar.slider("Identifier coverage (%)", 0, 100, 25) / 100
    )
    src_identifier_type = None
else:
    src_identifier_coverage = 1.0
    src_identifier_type = None

# Extra date
src_extra_date = st.sidebar.checkbox("Add entry date", value=False)

# Clinical variable
src_clinvar = st.sidebar.checkbox("Add clinical variable", value=False)

# Header

src_header_type = st.sidebar.radio(
    "Header type", ["Random", "Standard", "Fixed"], index=0
)
...
trg_size = st.sidebar.number_input("Number of samples", 10, 1000000, 1000)

# Linkage

st.sidebar.title("Ground truth")

exp_linkage_rate = st.sidebar.slider("Expected linkage rate (%)", 0, 100, 70) / 100

linkage_difficulty = st.sidebar.radio("Linkage difficulty", ["Low", "High"])
...
if not compute:
    ...
    # Truth params
    truth_params = {"exp_linkage_rate": exp_linkage_rate}
